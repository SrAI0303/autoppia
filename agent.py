...
def _classify_task(task: str) -> str:
    """
    Map a natural-language benchmark prompt to a stable playbook label for the policy LLM.

    Ordering (important): longer, site-specific, and compound phrases are matched before
    short generic keywords (e.g. login-then-action before bare "login") so the correct
    multi-step playbook is selected.

    Returns:
        Uppercase key into ``_TASK_PLAYBOOKS``, or ``GENERAL`` when no pattern matches.
    """
    t = (task or "").lower()
    ...
    # ---- AutoStats (8014) ----
    if re.search(r"disconnect\s+(the\s+)?wallet|wallet\s+with\s+wallet_name", t, re.IGNORECASE):
        return "DISCONNECT_WALLET"
    if re.search(
        r"\bconnect\w*\s+(a\s+)?wallet\b|\bconnected\s+wallet\b|show\s+details\s+for\s+connecting\s+a\s+wallet",
        t,
        re.IGNORECASE,
    ):
        return "CONNECT_WALLET"
    if re.search(r"favorite\s+the\s+subnet|subnet\s+where\s+the\s+subnet_name", t, re.IGNORECASE):
        return "FAVORITE_SUBNET"
    if re.search(r"details?\s+for\s+a\s+block\b|show\s+details\s+for\s+a\s+block", t, re.IGNORECASE):
        return "VIEW_BLOCK"
    if re.search(r"details?\s+for\s+a\s+subnet\b|show\s+details\s+for\s+a\s+subnet", t, re.IGNORECASE):
        return "VIEW_SUBNET"
    if re.search(r"details?\s+for\s+a\s+validator\b|show\s+details\s+for\s+a\s+validator", t, re.IGNORECASE):
    ...
    if re.search(r"open\s+direct\s+messages|\bview\s+dms?\b", t, re.IGNORECASE):
        return "VIEW_DMS"
    if re.search(r"settings?.*display\s+name|saving\s+a\s+display\s+name", t, re.IGNORECASE):
        return "SETTINGS_ACCOUNT"
    if re.search(r"open\s+(the\s+)?settings\b", t, re.IGNORECASE):
        return "OPEN_SETTINGS"
    if re.search(r"select\s+the\s+dm\b|select\s+.*\bdm\b.*where", t, re.IGNORECASE):
        return "SELECT_DM"
    if re.search(r"select\s+the\s+server\b", t, re.IGNORECASE):
...
_TASK_PLAYBOOKS: Dict[str, str] = {
    ...
    "SELECT_DATE": (
        "PLAYBOOK (any date picker—trip, hotel, restaurant, calendar event): "
        "1) Open the correct picker for the flow named in the task (check-in vs check-out vs trip date if both exist). "
        "2) Map constraints: equals→that exact calendar day; not_equals→any other valid day; "
        "after/before/greater/less→strictly inside the allowed range; on_or_after/on_or_before→inclusive at boundary. "
        "3) Use month/year controls until the target month is visible, then click the day cell. "
        "4) Confirm with Apply/OK if the widget requires it; verify the displayed date before continuing."
    ),
    "SELECT_TIME": (
        "PLAYBOOK (time slot, dropdown, or clock widget): "
        "1) Open the time control tied to the same booking/event step as the task. "
        "2) Interpret constraints numerically: less_equal→time ≤ bound; greater_equal→time ≥ bound; "
        "equals→match exactly including AM/PM. "
        "3) If only discrete slots exist, pick the first slot that satisfies the inequality. "
        "4) Confirm; verify the shown time matches the constraint."
    ),
    "NEXT_PICKUP": (
    ...
    "VIEW_PENDING_EVENTS": (
        "PLAYBOOK: 1) Open Calendar, Agenda, or a Pending/Upcoming events view (sidebar or tab). "
        "2) Use list_cards when rows show dates/titles. "
        "3) Select the event or row that satisfies ALL constraints (e.g. earliest date NOT equals X). "
        "4) Click to open details, or ensure the constrained pending list is visible (AutoDoc calendar vs AutoCalendar)."
    ),
    # ---- AutoList (8011) ----
    ...
    "EDIT_NUMBER_OF_GUESTS": (
        "PLAYBOOK: 1) When the task names listing constraints, use list_cards and open the hotel/listing that matches ALL of them "
        "(guests_to, rating, title, amenities, location, etc.). "
        "2) Otherwise, find the active listing or search form for the current flow. "
        "3) Open the guest/people selector (+/−, stepper, or dropdown). "
        "4) Set the count to the exact target from TASK_CONSTRAINTS; confirm if the UI applies changes immediately."
    ),
    "SUBMIT_REVIEW": (
    ...
    "CONTACT_FORM_SUBMIT": (
        "PLAYBOOK: 1) Navigate to Contact or Support (footer, nav, or help menu). "
        "2) Fill required fields: CONTAINS→include the literal substring; NOT CONTAINS / NOT equals→use any compliant value. "
        "3) On restaurant-style sites, align email/username fields with explicit constraints (domain fragment, excluded names). "
        "4) Submit; resolve validation errors before done."
    ),
    # ---- AutoShop (8002) ----
    "CATEGORY_FILTER": (
    ...
    # ---- AutoStats (8014) ----
    "CONNECT_WALLET": (
        "PLAYBOOK (chain / wallet UI): 1) Open Wallets or Connections from the main nav or dashboard. "
        "2) Use list_cards or visible rows to find the wallet whose wallet_name satisfies equals / NOT / contains constraints. "
        "3) Click that row or 'View details' / expand so the requested detail pane is visible. "
        "4) If the task is about connecting (not yet linked), use the Connect flow and pick a wallet name that satisfies NOT constraints."
    ),
    "DISCONNECT_WALLET": (
        "PLAYBOOK: 1) Go to Wallets or Connected accounts. "
        "2) Find the row where wallet_name matches the task (equals). "
        "3) Use Disconnect / Remove; confirm if prompted. "
        "4) Verify the wallet no longer appears as connected."
    ),
    "FAVORITE_SUBNET": (
        "PLAYBOOK: 1) Open the Subnets list or explorer table (search or scroll if needed). "
        "2) Find the subnet whose subnet_name satisfies CONTAINS / NOT equals / NOT contains per TASK_CONSTRAINTS. "
        "3) Click Favorite / Star / bookmark on that row only. "
        "4) Confirm the favorited state is shown."
    ),
    "VIEW_BLOCK": (
        "PLAYBOOK: 1) Navigate to Blocks or Explorer. "
        "2) Scan rows for block number, extrinsicsCount, eventsCount, epoch, and other fields in the constraints. "
        "3) Open the single block that satisfies ALL numeric and NOT constraints. "
        "4) Leave the block detail view open."
    ),
    "VIEW_SUBNET": (
        "PLAYBOOK: 1) Open Subnets or network overview. "
        "2) Match subnet_name, volume24h, and any other fields to TASK_CONSTRAINTS. "
        "3) Click the subnet to open its detail page."
    ),
    "VIEW_VALIDATOR": (
        "PLAYBOOK: 1) Open Validators list (sort or filter if available). "
        "2) Find the validator where rank, totalWeight, and other fields satisfy ALL constraints. "
        "3) Click to open validator details."
    ),
    # ---- AutoDiscord (8015) ----
    "JOIN_VOICE_CHANNEL": (
        "PLAYBOOK: 1) Join the server/channel context if the task implies it (sidebar). "
        "2) Locate the voice channel in the channel list (speaker/headphone icon). "
        "3) Click Connect / Join voice. "
        "4) Confirm you are connected (voice UI or status indicator)."
    ),
    "VOICE_MUTE_TOGGLE": (
        "PLAYBOOK: 1) Ensure you are in the voice channel. "
        "2) Find the microphone mute control in the voice panel or bottom bar. "
        "3) Toggle mute so the muted state matches the task (e.g. muted equals False → unmute)."
    ),
    "VIEW_DMS": (
        "PLAYBOOK: 1) Click Direct Messages, the @ / mail icon, or 'Friends' → Messages in the sidebar. "
        "2) Ensure the DM list or conversation panel is visible."
    ),
    "OPEN_SETTINGS": (
        "PLAYBOOK: 1) Open user/settings menu (gear or profile dropdown). "
        "2) Click Settings. "
        "3) Stop when the Settings page or main settings categories are visible."
    ),
    "SETTINGS_ACCOUNT": (
        "PLAYBOOK: 1) Open Settings → My Account or Profile. "
        "2) Find display name / username field. "
        "3) Enter a value that satisfies NOT equals / contains constraints; save if required. "
        "4) Confirm the saved state or success message if shown."
    ),
    "SELECT_DM": (
        "PLAYBOOK: 1) Open Direct Messages. "
        "2) In the conversation list, click the DM whose display name equals or contains the constrained name. "
        "3) Ensure that thread is active in the main pane."
    ),
    "SELECT_SERVER": (
        "PLAYBOOK: 1) Find the server list (left rail). "
        "2) Click the server icon or name per the task (first server, named server, or constrained). "
        "3) Wait for channels to load."
    ),
    "SELECT_CHANNEL": (
        "PLAYBOOK: 1) Under the active server, scan text channels. "
        "2) Click the channel required by the task (named in prompt or first channel if unspecified). "
        "3) Ensure message history area is visible."
    ),
    "SEND_MESSAGE": (
        "PLAYBOOK: 1) Focus the message input at the bottom of the active channel. "
        "2) Type content satisfying TASK_CONSTRAINTS (exact phrase, NOT contains, etc.). "
        "3) Press Enter or click Send. "
        "4) Confirm the message appears in the timeline."
    ),
    # --- General fallback ---
    "GENERAL": (
        "PLAYBOOK: Parse the task into a primary user goal and Preconditions (login, navigation). "
        "Use WEBSITE_CONTEXT and TASK_CONSTRAINTS to choose the correct screen, row, and field. "
        "Execute the shortest reliable path: navigate → fill → submit → verify UI feedback. "
        "If the first match fails, re-scan with list_cards or search_text rather than guessing. "
        "Use done only when the success criteria in the task are visibly satisfied."
    ),
}
...
def _detect_website(url: str) -> str:
    """Map URL port to website name for IWA demo sites (8000–8015)."""
...
def _website_context(website: str) -> str:
    ...
    ctx: Dict[str, str] = {
        ...
        "AutoStats": (
            "SITE: Blockchain / subnet explorer. NAV: Dashboard, Subnets, Validators, Blocks, Wallets. "
            "Tables list subnets (subnet_name, volume24h), validators (rank, totalWeight), blocks (number, epoch, counts). "
            "Wallet flows: connect, disconnect, wallet_name on each row. "
            "Use row click or View details to open a detail pane."
        ),
        "AutoDiscord": (
            "SITE: Chat client (Discord-like). LEFT: server icons; channels under server; voice channels with join. "
            "BOTTOM: message input; voice controls (mute/deafen). "
            "DMs: separate section or icon; Settings via gear / user menu. "
            "Click channel to focus; Send with Enter."
        ),
    }
...
def _llm_decide(
    *,
    ...
    playbook = _PLAYBOOK_GLOBAL_PREFIX + _TASK_PLAYBOOKS.get(task_type, _TASK_PLAYBOOKS["GENERAL"])
